...
@register.simple_tag
def get_marquee_categories(repeat: int = 3):
    """
    Returns active categories repeated N times for a seamless marquee loop.
    Safe to use in any template without passing context variables.
    """
    try:
        repeat = int(repeat)
    ...
    key = f"{_site_cache_version()}:mrq1:active_categories:repeat:{repeat}"
    data = cache.get(key)
    if data is None:
        base = list(
            Category.objects
